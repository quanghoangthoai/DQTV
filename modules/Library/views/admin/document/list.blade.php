...
<div class="card-body">
            ...
            <div class="col-2">
                <div class="form-group">
                    <select class="form-control" name="documentType" id="">
                        ...
                        <option value={{ $value['id'] }} {{old('documentType') === $value['id'] ? 'selected' : ''}}>{{ $value['name'] }}</option>
                        @endforeach
                        @endif
                    </select>
                </div>
    ...
    <table id="tree-table" class="table table-striped table-bordered">
        ...
        <tbody>
            ...
            <tr>
                {{-- {{ date('d/m/Y', strtotime($lib['created_at'])) }} --}}
                ...
                <td class="text-center"><a href="{{ route('executeSlug', $iDoc->category['slug'] . "/".$iDoc['slug'] ) }}" target="_blank">{{ $iDoc['name'] }}</a></td>
                <td class="text-center">{{ mod_library_get_document_type($iDoc['document_type'])['name'] }}</td>
                <td class="text-center">{{ $iDoc->category['name'] }}</td>
                <td class="text-center"><span data-popup="tooltip" title="{{ mod_library_format_date($iDoc['created_at'], 'd-m-Y H:i:s') }}">{{ mod_library_format_date($iDoc['created_at'], 'd-m-Y H:i:s') }}</span></td>
