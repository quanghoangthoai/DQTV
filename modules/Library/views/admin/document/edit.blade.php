        ...
        <div class="col-md-4 p-0">
            <div class="card-body">
                <div class="form-group row">
                    ...
                    <div class="col-sm-12">
                        <select id="document-category-select" class="form-control" name="category_id">
                            <option value="">Chọn danh mục</option>
                            @if ($listCat && count($listCat) > 0)
                            @foreach ($listCat as $k => $iCategory)
                            <option data-id="{{ $iCategory['format_type'] }}" value="{{ $iCategory['id'] }}" {{ old('category_id',$doc['category_id']) == $iCategory['id'] ? 'selected' : '' }}>{{$iCategory['prefix']}} {{ $iCategory['name'] }}</option>
                            @endforeach
                            @endif
                        </select>
                    </div>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-12">
                        <select class="form-control" id="document-type-select" name="document_type">
                            ...
                            <option data-type="{{ $value['type'] }}" value="{{ $value['id'] }}" {{ old('document_type', $doc['document_type']) == $value['id'] ? 'selected' : '' }}>{{ $value['name'] }}</option>
                            @endforeach
                            @endif
                        </select>
                    </div>
...
<script>
    $( document ).ready(function() {
        var format_type = $('#document-category-select option:selected').data('id')
        load_type_select(format_type);
    ...
    function load_type_select(format_type){
        switch (format_type) {
            case 1:
                $("#document-type-select option").each(function(i){
                    $('#document-type-select').show();
                    if($(this).data("type") == 'isMultimedia')
                    $(this).hide()
                    else
                    $(this).show()
                });
            ...
            $("#document-type-select option").each(function(i){
                ...
                $('#document-type-select').prop('selectedIndex',0);
                $('#document-extend-text').hide();
    ...
    $("#document-category-select").on('change', function(){
        $('#document-type-select').prop('selectedIndex',0);
        $('#document-extend-text').hide();
        $('#document-extend-video').hide();
        var format_type = $(this).find(':selected').data('id')
        load_type_select(format_type);
    })
